...
The histograms of each asset show the distribution for the daily return of each assets. The more volatile the return is, the more risk the investor will bear. Also, the higher return also involves more risk of loss. 
Here we calculate the variance of each asset's return and find the more risky assets: VNQ and EEM, and the safer assets: LQD, TLT, SPY.
...
```
The optimum distribution of wealth by this simulation is calculated as: 
_Note how there are only two ETFs present in this portfolio_
...
```{r echo=FALSE}
library(corrplot)
social_mkting_df = read.csv('../data/social_marketing.csv', header=TRUE, row.names = 1)
